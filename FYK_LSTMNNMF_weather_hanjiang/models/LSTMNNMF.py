...
class LSTMNNMF(Model):
    ...
    def __init__(self, training_set, training_ground_truth, train_mask, val_mask, 
                 A, rank, time_lags, weather_data, lambda_w=100, lambda_x=100, eta=0.02, 
        ...
        # 加入气象数据（7个气象特征）
        self.weather_data = tf.constant(weather_data, dtype=tf.float32)  # 形状为 (11274, 7)
        
        # LSTM和全连接层
        self.LSTM = layers.LSTM(self.rank, input_shape=(self.lag_len + 7, self.rank), unroll=True)
    ...
    def create_lstm_inputs_vectorized(self):
        ...
        weather_data_resized = self.weather_data[:self.X_tf.shape[0]]  # 确保天气数据的时间步数与水质数据一致
        combined_data = tf.concat([self.X_tf, weather_data_resized], axis=-1)  # 拼接后的形状为 (num_times, rank + 7)
    
        # 按照索引提取拼接后的数据
        gathered = tf.gather(combined_data, flat_indices, axis=0)
    
        # 将数据 reshape 为 LSTM 输入形状
        lstm_inputs_with_weather = tf.reshape(gathered, [data_len, self.lag_len, self.rank + 7])
        
        print(f"LSTM输入创建完成: 形状={lstm_inputs_with_weather.shape}")
        
        return lstm_inputs_with_weather
    ...
    def loss_cal(self):
        ...
        try:
            ...
            # 2. L2正则化项
            W_F_norm = tf.math.square(tf.norm(self.W_tf, ord='fro', axis=(0, 1))) *  self.eta
            X_F_norm = tf.math.square(tf.norm(self.X_tf, ord='fro', axis=(0, 1))) *  self.eta
