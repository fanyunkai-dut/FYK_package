#第四步，把原始缺失文件生成个新的只有1和NaN的npy，标记哪个位置有缺失值，叫做WQ_hanjiang_binary.npy
import numpy as np
import yaml

def generate_binary_npy(config_path):
    """
    根据原始 .npy 文件生成一个新的二进制 .npy 文件，NaN 对应 NaN，非 NaN 对应 1。
    """
    # 读取配置文件
    ...
    # 获取原始 .npy 文件路径
    input_npy_path = config["WQ_data"]["input_file1"]
    ...
    binary_data = np.where(np.isnan(data),np.nan, 1)
