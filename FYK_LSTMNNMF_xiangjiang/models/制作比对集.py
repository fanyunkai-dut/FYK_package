#第七步，制作比对集，用原数据和1NaN掩膜相乘，再0.8/0.2划分，叫做WQ_hanjiang_true_train.npy；WQ_hanjiang_true_test.npy。
import numpy as np
import yaml
import os
...
def process_data(config_path):
    ...
    binary_npy_path = config["WQ_data"]["input_file2"]  # 二进制掩膜文件路径
    ...
    # 保存训练集和测试集
    train_output_path = input_npy_path.replace(".npy", "_true_train.npy")
    test_output_path = input_npy_path.replace(".npy", "_true_test.npy")
