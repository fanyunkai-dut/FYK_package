...
# 设置文件夹路径
folder_path = "/home/fanyunkai/FYK_data/QX_hanjiang/hanjiang_preci"
output_file = "/home/fanyunkai/FYK_data/QX_hanjiang/hanjiang_precipitation_4hours.csv"
...
for file in nc_files:
    ...
    precipitation_values.append(total_precipitation)

# 转换成DataFrame
df = pd.DataFrame({
    "time": times,
    "value": precipitation_values
})

# 对每一天的8个数据点进行线性插值
interpolated_times = []
interpolated_precipitation = []
...
for i in range(0, len(df), 8):
    ...
    if len(day_data) == 8:
        ...
        precipitation_of_day = day_data["precipitation"].values
        
        # 线性插值：我们要插值到0, 4, 8, 12, 16, 20点
        interpolation_func = interpolate.interp1d(times_of_day, precipitation_of_day, kind="linear")
        new_times = np.array([0, 4, 8, 12, 16, 20])
        new_precipitation = interpolation_func(new_times)
        
        # 记录插值结果，保留一位小数
        for new_time, precip in zip(new_times, new_precipitation):
            interpolated_times.append(f"{day_data['time'].iloc[0][0:7]}.{new_time:02d}")
            interpolated_precipitation.append(round(precip, 1))  # 保留一位小数

# 创建最终的DataFrame
final_df = pd.DataFrame({
    "time": interpolated_times,
    "precipitation": interpolated_precipitation
})
